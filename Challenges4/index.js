...
let array = arrays.reduce((prev,curr)=>prev.concat(curr), []);

console.log(array);
// [1, 2, 3, 4, 5, 6] 

// Challenge 2
function loop(value, condition, afterthought, statment) {
	if (!condition(value)) {
		return;
	} else {
		statment(value);
		const result = afterthought(value);
		loop(result, condition, afterthought, statment);
	}
}
...
function dominantDirection(text) {
	return countBy(text.split(""), char => {
		let script = characterScript(char.codePointAt(0));
		return script ? script : "none";
	}).filter(({name}) => name !== "none")[0].name.direction;
}
